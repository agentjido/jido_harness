defmodule Jido.Harness.PiRPCSessionTest do
  ...
  test "Pi RPC preserves context and supports native steering and configuration" do
    assert {:ok, session_id} = Jido.Harness.open_session(:pi)
    assert {:ok, %{provider_session_id: "pi-fixture-session", transport: :rpc}} = await_ready(session_id)

    assert :ok = Jido.Harness.configure_session(session_id, %{reasoning_effort: :high})

    assert {:error, %Jido.Harness.Error{details: %{field: :approval_mode}}} =
             Jido.Harness.configure_session(session_id, %{approval_mode: :auto_approve})

    assert {:ok, first_id} = Jido.Harness.send_message(session_id, %{prompt: "first", reasoning_effort: :low})
    assert {:ok, %{status: :completed, text: "fixture-1"}} = Jido.Harness.await_turn(session_id, first_id, 2_000)

    assert {:ok, held_id} = Jido.Harness.send_message(session_id, "hold")

    assert {:error, %Jido.Harness.Error{details: %{field: :reasoning_effort}}} =
             Jido.Harness.steer(session_id, %{prompt: "ignored", reasoning_effort: :high})

    assert {:ok, _request_id} = Jido.Harness.steer(session_id, "finish")
    assert {:ok, %{status: :completed, text: "steered"}} = Jido.Harness.await_turn(session_id, held_id, 2_000)
  ...
  test "Pi interruption leaves the session usable for another turn" do
    ...
    assert {:ok, held_id} = Jido.Harness.send_message(session_id, "hold")
    assert :ok = Jido.Harness.interrupt_turn(session_id, held_id)
    assert {:ok, %{status: :interrupted}} = Jido.Harness.await_turn(session_id, held_id, 2_000)

    assert {:ok, next_id} = eventually_send(session_id, "next")
    assert {:ok, %{status: :completed, text: "fixture-1"}} = Jido.Harness.await_turn(session_id, next_id, 2_000)
  ...
  defp eventually_send(session_id, input, attempts) do
    case Jido.Harness.send_message(session_id, input) do
      {:error, :busy} ->
        Process.sleep(10)
