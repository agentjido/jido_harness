defmodule Jido.Harness.ACPSessionTest do
  ...
  test "ACP correlates fragmented JSONL responses and preserves context identity" do
    assert {:ok, session_id} = Jido.Harness.open_session(:kimi, %{})
    assert {:ok, %{provider_session_id: "acp-fixture-session", transport: :acp}} = await_ready(session_id)

    assert {:ok, first_id} = Jido.Harness.send_message(session_id, "first")
    assert {:ok, %{status: :completed, text: "fixture-ok"}} = Jido.Harness.await_turn(session_id, first_id, 2_000)

    assert {:ok, second_id} = Jido.Harness.send_message(session_id, "invalid frame")
    assert {:ok, %{status: :completed, text: "fixture-ok"}} = Jido.Harness.await_turn(session_id, second_id, 2_000)
  ...
  test "ACP translates permission requests and rejects stale responses" do
    ...
    assert {:ok, turn_id} = Jido.Harness.send_message(session_id, "request approval")
    assert {:ok, %{pending_approvals: 1}} = await_approval(session_id)

    assert {:ok, events} = Jido.Harness.replay_session(session_id, limit: 1_000)
    request_id = Enum.find(events, &(&1.type == :approval_requested)).request_id
    assert :ok = Jido.Harness.respond_approval(session_id, request_id, :approve)
    assert {:error, :not_found} = Jido.Harness.respond_approval(session_id, request_id, :deny)
    assert {:ok, %{status: :completed, text: "approved"}} = Jido.Harness.await_turn(session_id, turn_id, 2_000)
  end

  test "approval timeouts deny the provider request without closing the session" do
    assert {:ok, session_id} = Jido.Harness.open_session(:kimi, %{approval_timeout_ms: 25})
    assert {:ok, _info} = await_ready(session_id)
    assert {:ok, turn_id} = Jido.Harness.send_message(session_id, "request approval")
    assert {:ok, %{status: :completed, text: "denied"}} = Jido.Harness.await_turn(session_id, turn_id, 2_000)
    assert {:ok, %{state: :idle, pending_approvals: 0}} = Jido.Harness.info_session(session_id)
  ...
  test "duplicate approval notifications replace their timer without racing the response" do
    assert {:ok, session_id} = Jido.Harness.open_session(:kimi, %{approval_timeout_ms: 500})
    assert {:ok, _info} = await_ready(session_id)
    assert {:ok, turn_id} = Jido.Harness.send_message(session_id, "duplicate approval")
    assert {:ok, %{pending_approvals: 1}} = await_approval(session_id)
    ...
    assert :ok = Jido.Harness.respond_approval(session_id, hd(approvals).request_id, :approve)
    assert {:ok, %{status: :completed, text: "approved"}} = Jido.Harness.await_turn(session_id, turn_id, 2_000)
    Process.sleep(550)
    assert {:ok, %{state: :idle, pending_approvals: 0}} = Jido.Harness.info_session(session_id)
  end
  ...
  test "ACP rejects ignored session and turn options before dispatch" do
    assert {:error, %Jido.Harness.Error{details: %{field: :model}}} =
             Jido.Harness.open_session(:kimi, %{model: "ignored"})

    assert {:error, %Jido.Harness.Error{message: "unknown provider option"}} =
             Jido.Harness.open_session(:kimi, %{provider_options: %{extra_args: ["--unsafe"]}})

    assert {:ok, session_id} = Jido.Harness.open_session(:kimi)
    assert {:ok, _info} = await_ready(session_id)

    assert {:error, %Jido.Harness.Error{details: %{field: :reasoning_effort}}} =
             Jido.Harness.send_message(session_id, %{prompt: "hello", reasoning_effort: :high})
  end
