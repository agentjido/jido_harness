...
The audit continues to run and report its findings. The project uses Hex's
`ignore_advisories` setting for only these three IDs, so a new advisory still
fails the audit. Remove the entries and recheck the audit when a supported
dependency fix becomes available. Removing HTTP dependencies is follow-up work
and is not required to complete this PR under the exception.
...
A Cowlib update alone is not currently sufficient. Do not add other advisories
to the temporary exception, use an unverified Git revision, or rely only on an
aggregate CI result. No HTTP-adapter change or dependency release is included
in this Harness PR.
